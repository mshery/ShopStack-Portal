...
import type { ProductCategory } from "@/shared/types/models";
...
export default function CategoriesPage() {
    ...
    const tenantCategories = useMemo(() => {
        const list = categories.filter((c) => c.tenant_id === activeTenantId);
        if (!search) return list;
        return list.filter((c) =>
            c.name.toLowerCase().includes(search.toLowerCase())
        );
    }, [categories, activeTenantId, search]);

    const totalPages = Math.ceil(tenantCategories.length / ITEMS_PER_PAGE);
    const paginatedCategories = useMemo(() => {
        const start = (currentPage - 1) * ITEMS_PER_PAGE;
        return tenantCategories.slice(start, start + ITEMS_PER_PAGE);
    }, [tenantCategories, currentPage]);
    ...
    return (
        <div className="space-y-6 p-6">
            ...
            {/* Content */}
            {tenantCategories.length === 0 && !isAddingNew ? (
                ...
                <Card className="rounded-2xl border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    <CardContent className="p-0">
                        ...
                        <Pagination
                            ...
                            totalItems={tenantCategories.length}
